...
\item Recorded the rendering environment in
  \code{replication/SESSION\_INFO.txt}, captured via
...
\code{replication/SESSION\_INFO.txt}.
...
\verb|curves <- read.delim("https://raw.githubusercontent.com/|
\verb|JeromeMathieuEcology/EGrowth/master/curves.txt")|. This file
should also be directly included in the replication material to
reduce the dependency on availability from external unreliable
sources.''
\end{displayquote}
...
\verb|Error in bdeb_model(): Temperature list missing: T_obs.|''
\end{displayquote}
...
\verb|'bdeb_model' has a help file but no examples|. Since
