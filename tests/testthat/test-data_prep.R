...
# --- Survival validation: edge cases ---

test_that("bdeb_data validates survival data", {
  df <- data.frame(id = 1:3, time = c(5, 10, 15), event = c(1, 0, 1))
  dat <- bdeb_data(survival = df)
  expect_equal(dat$endpoints, "survival")
  expect_equal(dat$n_ind, 3)
})

test_that("bdeb_data rejects invalid survival events", {
  df <- data.frame(id = 1, time = 5, event = 2)
  expect_error(bdeb_data(survival = df), "0.*1")
})

test_that("bdeb_data survival missing columns gives error", {
  df <- data.frame(id = 1, time = 5)
  expect_error(bdeb_data(survival = df), "missing columns")
})
