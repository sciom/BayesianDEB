...
##
## Output:
##   sbc_growth_repro_results.rds  — archived results
##   (console)                     — chi-squared tests & coverage
##
## Runtime: ~2–4 hours for 100 replications on 8-core machine
## =============================================================
...
## --- Priors (E. fetida base + reproduction parameters) ---------------
## Same base priors as individual SBC (Table sbc_priors in paper).
## NOTE: Stan model has <lower=0> constraint on prior_*_mu for repro
## parameters, so we use package defaults which satisfy this.

## Build priors manually.  The core DEB priors match the individual
## SBC (Table sbc_priors).  Reproduction priors are calibrated so
## that k_J * E_Hp << (1-kappa) * p_C at typical sizes, ensuring
## positive reproduction flux in most draws.
sbc_priors <- list(
